...
function checkAchievements() {
  ...
  const epicCount = state.quests.filter(
    (q) => q.type === "epic" && q.completed,
  ).length;
...
function updateSidebar() {
  ...
  document.getElementById("mini-char-level").textContent =
...
function updateDashboard() {
  ...
  document.getElementById("dash-level-text").textContent =
    state.character.level;
  document.getElementById("dash-class-name").textContent =
  ...
  document.getElementById("stat-completed-num").textContent =
    state.character.lifetime.questsDone;
  document.getElementById("stat-notes-num").textContent = state.notes.length;
  document.getElementById("stat-trophies-num").textContent =
  ...
  if (activeFirst.length > 0) {
    recentQuests.innerHTML = activeFirst
      .map(
        (q) => `
...
function updateCharacterSheet() {
  ...
  document.querySelectorAll(".avatar-option").forEach((btn) => {
    btn.classList.toggle(
      "active",
      btn.dataset.avatar === state.character.avatar,
    );
  });

  // Lifetime
  document.getElementById("lt-total-xp").textContent =
    state.character.lifetime.xpEarned;
  document.getElementById("lt-quests-done").textContent =
    state.character.lifetime.questsDone;
  document.getElementById("lt-notes-written").textContent =
    state.character.lifetime.notesWritten;
  document.getElementById("lt-best-streak").textContent =
    state.character.lifetime.bestStreak;
}

function updateTrophySpace() {
  document.getElementById("trophy-count-display").textContent =
    `${state.achievements.length} / ${Object.keys(ACHIEVEMENTS_DB).length}`;

  const grid = document.getElementById("achievements-grid");
  grid.innerHTML = Object.keys(ACHIEVEMENTS_DB)
    .map((key) => {
...
function showLevelUpModal() {
  ...
  document.getElementById("levelup-level-num").textContent =
    state.character.level;
  document.getElementById("levelup-class-name").textContent =
    state.character.class;
...
function setupQuestLogic() {
  ...
  document
    .getElementById("quest-modal-close")
    .addEventListener("click", closeAllModals);
  document
    .getElementById("quest-modal-cancel")
    .addEventListener("click", closeAllModals);
  ...
  document.getElementById("btn-add-subtask").addEventListener("click", () => {
    ...
    div.innerHTML = `
        ...
        `;
    div
      .querySelector(".btn-remove-subtask")
  ...
  document.querySelectorAll(".quest-filters .filter-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      document
        .querySelectorAll(".quest-filters .filter-btn")
        .forEach((b) => b.classList.remove("active"));
...
window.completeQuest = (questId) => {
  ...
  if (q) {
    ...
    let statBoost =
      q.type === "epic" ? "str" : q.type === "main" ? "vit" : null;
...
function setupNoteLogic() {
  ...
  document
    .getElementById("note-modal-close")
    .addEventListener("click", closeAllModals);
  document
  ...
  document.querySelectorAll(".note-categories .cat-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      document
        .querySelectorAll(".note-categories .cat-btn")
        .forEach((b) => b.classList.remove("active"));
      e.currentTarget.classList.add("active");
...
window.openNote = (id) => {
  ...
  if (n) {
    ...
    document
      .querySelectorAll(".color-option")
      .forEach((c) => c.classList.remove("active"));
    const clrBtn = document.querySelector(
      `.color-option[data-color="${n.color}"]`,
    );
...
function initParticles() {
  ...
  class Particle {
    ...
    reset() {
      ...
      this.color =
        Math.random() > 0.5 ? "rgba(167, 139, 250," : "rgba(251, 191, 36,";
    }
    update() {
      this.x += this.speedX;
      this.y += this.speedY;
